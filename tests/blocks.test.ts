...
function structuredArtifact(overrides = {}) {
	return {
		...
		contextMap: [{ source: "/repo/AGENTS.md", relevance: "repo operating guide", use: "verify runtime boundaries" }],
		...
		durablePromotions: [{
			...
			durability: "Agent guide owns runtime invariants.",
			risk: "Prompt/docs drift if left only in assets.",
			nextAction: "Update AGENTS.md before delivery.",
		}],
...
test("parseHistoryArtifacts requires the v3 structured JSON artifact contract", () => {
	...
	assert.deepEqual(parseHistoryArtifacts(validArtifacts), {
		continuation: [
			...
			"## Context Map\n- /repo/AGENTS.md — repo operating guide; use it to verify runtime boundaries.",
			"## Working Edge\n- Update tests and docs, then run the gate.",
			...
			"## Durable Promotions\n- apply: AGENTS.md — Record continuation ledger conservation semantics.; evidence: Prompt contract now preserves initiative spine and durable promotions.; durability: Agent guide owns runtime invariants.; risk: Prompt/docs drift if left only in assets.; next: Update AGENTS.md before delivery.",
