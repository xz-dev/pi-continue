...
import { DEFAULT_CONTINUE_CONFIG, loadContinuationConfig, resetContinuationConfig, saveContinuationConfig } from "../extensions/continue/src/config.ts";
...
test("loadContinuationConfig preserves explicit repo continuation sync", async () => {
	await withTempAgent(async (root) => {
		const configDir = join(root, ".pi", "extensions");
		mkdirSync(configDir, { recursive: true });
		writeFileSync(join(configDir, "pi-continue.json"), JSON.stringify({ continuationDocSyncMode: "always" }), "utf8");
		const config = loadContinuationConfig(root);
